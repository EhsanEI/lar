...
class MNISTM(data.Dataset):
    ...
    # url = "https://github.com/keras_mnistm/releases/download/1.0/keras_mnistm.pkl.gz"
    url = "file:///home/data/keras_mnistm.pkl.gz"
    ...
    def __init__(self,
                 ...
                 download=False, neg_class=0, pos_class=1):
        ...
        if self.train:
            self.train_data, self.train_labels = \
                torch.load(os.path.join(self.root,
                                        ...
                                        self.training_file))
            self.train_data, self.train_labels = binary_imbalanced(
                self.train_data, self.train_labels, neg_class=params.classes[0], pos_class=params.classes[1], ratio=None)
            print('mnistm train', self.train_data.shape, self.train_labels.shape, self.train_labels.min(), self.train_labels.max(), self.train_labels.float().mean())
        else:
            self.test_data, self.test_labels = \
                torch.load(os.path.join(self.root,
                                        ...
                                        self.test_file))
                                        # self.training_file))
            self.test_data, self.test_labels = binary_imbalanced(
                self.test_data, self.test_labels, neg_class=params.classes[0], pos_class=params.classes[1], ratio=None)
            print('mnistm test', self.test_data.shape, self.test_labels.shape, self.test_labels.min(), self.test_labels.max(), self.test_labels.float().mean())
...
mnistm_train_dataset = MNISTM(root='~/data/mnistm', train=True, download=True,
                              transform=transform)
mnistm_valid_dataset = MNISTM(root='~/data/mnistm', train=True, download=True,
                              transform=transform)
mnistm_test_dataset = MNISTM(root='~/data/mnistm', train=False, transform=transform)

indices = list(range(len(mnistm_train_dataset)))
validation_size = 0  # 5000
