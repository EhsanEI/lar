...
with open(os.path.join('results_reg', f"synthetic_None_results.json"), 'r') as f:
    results = json.load(f)

with open(os.path.join('results_reg', f"synthetic_None_configs.json"), 'r') as f:
    configs = json.load(f)
...
for config in configs:
    this_result = [mse for mse in results['test_acc'][config_str(config)]]
...
plt.ylabel('Target Domain MSE')
# plt.legend()
plt.tight_layout()
plt.savefig('synthetic_reg.png')