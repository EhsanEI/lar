...
for run in range(meta_data['num_runs']):
    ...
    X_src, y_src, X_tgt, y_tgt, X_val, y_val = get_data_synthetic_clf(meta_data, seed=run, classification=False)
    ...
    for config in configs:
            ...
            if plot_curves or (epoch == meta_data['num_epoch'] - 1):
                ...
                with torch.no_grad():
                    ...
                    if torch.isnan(yhat).any():
                        ...
                        yhat = yhat > 0
                        mse = loss_sup(yhat, y_src_ten).item()
                    train_curve.append(mse)
                    ...
                    if torch.isnan(yhat).any():
                        ...
                        yhat = yhat > 0
                        mse = loss_sup(yhat, y_tgt_ten).item()
                    test_curve.append(mse)
                    ...
                    if torch.isnan(yhat).any():
                        ...
                        yhat = yhat > 0
                        mse = loss_sup(yhat, y_val_ten).item()
                    val_curve.append(mse)
        ...
        if plot_figs and run == 0:
            ...
            for (X, y, title) in [(X_src, y_src, 'labeled'), 
                ...
                plt.scatter(X[:, 0], X[:, 1], marker='o', c=y, cmap='coolwarm', alpha=1., edgecolors='none')
                ...
                if config_str(config) == config_str(configs[0]):
                    ...
                    # plt.legend()
                    plt.arrow(0, 0, w_np[0, 0], w_np[0, 1], color='darkgreen', width=.06, label='No Adaptation')
                    plt.arrow(0, 0, no_adapt_w[0, 0], no_adapt_w[0, 1], color='purple', width=.06, label='Regularized')
                    # plt.arrow(0, 0, wstar[0], wstar[1], color='royalblue', width=.06, label='optimal')
...
print('saving')
with open(os.path.join('results_reg', f"synthetic_{meta_data['ratio']}_results.json"), 'w') as f:
    json.dump(results, f)

with open(os.path.join('results_reg', f"synthetic_{meta_data['ratio']}_configs.json"), 'w') as f:
    json.dump(configs, f)
